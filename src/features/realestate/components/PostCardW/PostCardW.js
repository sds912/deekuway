...
import { getLastSelectedPost, saveAlreadyViewPost, saveLastSelectedPost } from '../../services/localStorageService';
import { CloseCircleOutlined, DeleteOutlined, EditOutlined, ExclamationCircleOutlined, EyeFilled, EyeOutlined, LikeOutlined, MoreOutlined, SettingOutlined } from '@ant-design/icons';
...
export const PostCardW = ({
   ...
   const { Text } = Typography;
    ...
    return  <Card
    className={ (lastViewedPost && (lastViewedPost === post.id)) ? 'post-card post-card-active':'post-card'}
    bodyStyle={{
      padding: '6px', 
      position: 'relative',
    ...
    onClick={()=>
     { 
	  ...
	  saveLastSelectedPost(post.id);
	  updateLastViewedPost( getLastSelectedPost())
