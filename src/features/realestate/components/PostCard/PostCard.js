...
import { CloseCircleOutlined, DeleteOutlined, EditOutlined, ExclamationCircleOutlined, EyeFilled, HeartOutlined, LikeOutlined, MoreOutlined} from '@ant-design/icons';
	  ...
	  const addToFavorites = async (event, post) => {
		...
		try {
		 ...
		 // reloadData();
		  ...
		  (await userRef).docs.forEach(doc => {

			if(doc.data().likes && doc.data().likes.includes(post.id)){
				firebase
				.firestore()
				.collection('user')
				.doc(doc.id)
				.update({likes: firebase.firestore.FieldValue.arrayRemove(post.id)}); 
				
				message.success('Retiré de vos favories !');
				if(post.like >= 1){
				    const p = post;
					 p.like --;
					 console.log(p.like)
				...
				.collection('user')
				.doc(doc.id)
				.update({likes: firebase.firestore.FieldValue.arrayUnion(post.id)});
		...
		<div className="PostCard">
			...
			<Card
				...
				<button
				    ...
				    style={{position: 'absolute', top: '8px', right: '6px', fontSize:'22px', backgroundColor:'inherit', border: 'none'}}>
					<div 
					style={{
						display: 'relative',
						...
						<HeartOutlined style={{color: inFavorite ? 'orange': null, fontSize: '18px'}}  />
					</div>
				</button>}
